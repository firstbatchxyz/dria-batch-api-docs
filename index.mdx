...
- Programmatically poll the Batch Status API and download results once ready (example below)
- Or, if available, use additional API endpoints for status/result retrieval (see dashboard or mail inference@dria.co)

The snippet below reuses the `file_id` returned during the upload flow to poll for status updates and download the completed batch output:

```python
import time
import requests

def wait_for_batch(file_id, api_key, base_url, interval=30):
    headers = {'x-api-key': api_key}
    while True:
        resp = requests.get(f'{base_url}/batch/status', params={'id': file_id}, headers=headers)
        resp.raise_for_status()
        data = resp.json()
        print(f"Batch {file_id} status: {data['status']}")
        if data['status'] in {'COMPLETED', 'FAILED'}:
            return data
        time.sleep(interval)

status = wait_for_batch(file_id, api_key, base_url)
if status['status'] == 'COMPLETED':
    result = requests.get(f'{base_url}/batch/result', params={'id': file_id}, headers={'x-api-key': api_key})
    result.raise_for_status()
    with open('batch_result.jsonl', 'wb') as f:
        f.write(result.content)
    print('✅ Results downloaded to batch_result.jsonl')
else:
    print('❌ Batch failed:', status)
```
